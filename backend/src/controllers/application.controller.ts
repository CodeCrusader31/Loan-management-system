...
export const uploadSalarySlip = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let salarySlipUrl = req.body.salarySlipUrl;

    // ✅ If file uploaded via Cloudinary
    if (req.file) {
      salarySlipUrl = (req.file as any).path; // Cloudinary URL
    }
    ...
    res.status(200).json(
      new ApiResponse(200, application, 'Salary slip uploaded successfully')
    );
  } catch (error) {
    next(error);
  }
